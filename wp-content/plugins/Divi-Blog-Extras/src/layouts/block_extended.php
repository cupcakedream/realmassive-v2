...
if( $counter == 1 ){
    ...
    if ( '' !== $show_categories ) {
        ...
        if ( '' !== $category_hover_background ) {
            ET_Builder_Element::set_style( $function_name, array(
                'selector'    => '%%order_class%% .post-categories a:hover',
                'declaration' => sprintf(
                    'background-color: %1$s;',
    ...
    if( 'background' === $image_position || $image_position == 'alternate' ){
...
if ( 'on' === $show_categories ) {
    ...
    ?>
    <!--<div class="post-categories"><?php //echo $category_list; ?></div>-->
    <?php
}
...
if ( 'on' === $show_author || 'on' === $show_date || 'on' === $show_comments ) {
    printf( '<p class="post-meta">%1$s %2$s %3$s %4$s %5$s</p>',
        (
        'on' === $show_author
            ? '<span class="author vcard"><span class="et-pb-icon">&#xe08a;</span>'.et_get_safe_localization( sprintf( __( ' %s', 'et_builder' ),  et_pb_get_the_author_posts_link() . '</span>' ) )
            : ''
        ),
        (
