 ...
 *
 * This template can be overridden by copying it to yourtheme/divi-blog-extras/ajax-layouts/box_extended.php.
 *
...
$html .= '<div class="post-content">';
    ...
    if ( 'on' === $show_content ) {
        ...
        if ( has_excerpt() ) {
            $html .= el_blog_strip_shortcodes( get_the_excerpt($pid) );
        } else {
            if( $excerpt_length != 0 ){
                $html .= wpautop( strip_shortcodes( el_blog_truncate_post( $excerpt_length, false, $post_object, true ) ) );
            }
        }
    ...
    if ( 'on' === $show_author || 'on' === $show_date || 'on' === $show_comments ) {
        $html .= sprintf( '<p class="post-meta">%1$s %2$s %3$s %4$s %5$s</p>',
            (
            'on' === $show_author
                ? '<span class="author vcard">'.get_avatar( get_the_author_meta('ID') , 28 ).et_get_safe_localization( sprintf( __( ' %s', 'et_builder' ),  get_the_author_posts_link()  ) ). '</span>'
                : ''
            ),
            (
