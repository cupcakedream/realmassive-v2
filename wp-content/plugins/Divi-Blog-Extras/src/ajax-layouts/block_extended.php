 ...
 *
 * This template can be overridden by copying it to yourtheme/divi-blog-extras/ajax-layouts/block_extended.php.
 *
...
if ( 'on' === $show_categories ) {
    ...
    if( 'on' === $category_meta_colors ){
        foreach( $categories as $category ){
            ...
            $br_style       = ( '' !== $bgcolor )   ? 'border-color: '.$bgcolor.';' : '';
            ...
            $custom_style   = ' style="'.$color_style.$bg_style.$br_style.'"';
    ...
    $html .= '<div class="post-categories">'. $category_list .'</div>';
}
...
$html .= sprintf( '<p class="post-meta">%1$s %2$s %3$s %4$s %5$s</p>',
    (
    'on' === $show_author
        ? et_get_safe_localization( sprintf( __( '%s', 'et_builder' ), '<span class="author vcard"><span class="et-pb-icon">&#xe08a;</span>' .  get_the_author_posts_link() . '</span>' ) )
        : ''
    ),
    (
    ...
    (
    'on' === $show_date
        ? et_get_safe_localization( sprintf( __( '%s', 'et_builder' ), '<span class="published"><span class="et-pb-icon">&#xe06b;</span>' . esc_html( get_the_date( $meta_date, $pid ) ) . '</span>' ) )
        : ''
    ),
    (
    ...
    (
    'on' === $show_comments
        ? '<span class="et-pb-icon">&#xe065;</span>'.sprintf( esc_html( _nx( '1', '%s', get_comments_number(), 'number of comments', 'et_builder' ) ), number_format_i18n( get_comments_number() ) )
        : ''
    )
);
}
