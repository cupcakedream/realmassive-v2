 ...
 *
 * This template can be overridden by copying it to yourtheme/divi-blog-extras/ajax-layouts/classic.php.
 *
...
if ( 'on' === $show_categories || 'on' === $show_date ) {
    ...
    $html .= sprintf( '<p class="post-meta">%1$s %2$s %3$s</p>',
    		(
    			'on' === $show_categories
    		...
    		(
    			( 'on' === $show_categories && 'on' === $show_date )
    				? ' - '
    				: ''
    		),
    		(
...
if ( 'on' === $show_author || 'on' === $show_social_icons || 'on' === $show_comments ) {
    $html .= sprintf( '<p class="post-meta">%1$s %2$s %3$s %4$s %5$s</p>',
        (
        'on' === $show_author
            ? et_get_safe_localization( sprintf( __( '%s', 'et_builder' ), '<span class="author vcard">' .  get_the_author_posts_link() . '</span>' ) )
            : ''
        ),
        (
        ...
        'on' === $show_social_icons
            ...
            <span class="et-social-icon et-social-facebook"><a href="https://www.facebook.com/sharer.php?u='.get_the_permalink($pid).'&t='.get_the_title($pid).'" class="icon"></a></span>
            <span class="et-social-icon et-social-twitter"><a href="https://twitter.com/share?text='.get_the_title($pid).'&url='.get_the_permalink($pid).'" class="icon"></a></span>
            <span class="et-social-icon et-social-google-plus"><a href="https://plus.google.com/share?url='.get_the_permalink($pid).'" class="icon"></a></span>
            </span>'
            : ''
        ),
        (
