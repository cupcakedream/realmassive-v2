...
function DS_PBWID_activate_license() {
	// retrieve the license from the database
	$license = trim( get_option( 'DS_PBWID_license_key' ) );

	// data to send in our API request
	$api_params = array(
		'edd_action' => 'activate_license',
		'license'    => $license,
		'item_name'  => urlencode( DS_PBWID_ITEM_NAME ), // the name of our product in EDD
		'url'        => home_url()
	);

	// Call the custom API.
	$response = wp_remote_post( DS_PBWID_STORE_URL, array( 'timeout' => 15, 'sslverify' => false, 'body' => $api_params ) );
		...
		if ( false === $license_data->success ) {
			...
			switch( $license_data->error ) {
				...
				case 'item_name_mismatch' :
					...
					$message = sprintf( __( 'This appears to be an invalid license key for %s.', 'aspengrove-updater' ), DS_PBWID_ITEM_NAME );
					break;
	...
	if ( ! empty( $message ) ) {
		delete_option('DS_PBWID_license_key');
	
		$base_url = admin_url( DS_PBWID_PLUGIN_PAGE );
	...
	update_option( 'DS_PBWID_license_status', $license_data->license );
	wp_redirect( admin_url( DS_PBWID_PLUGIN_PAGE ) );
	exit();
}

function DS_PBWID_deactivate_license() {

	// run a quick security check
	if( ! check_admin_referer( 'DS_PBWID_license_key_deactivate', 'DS_PBWID_license_key_deactivate' ) )
	...
	if ($license_data->license == 'deactivated') {
		delete_option('DS_PBWID_license_status');
		delete_option('DS_PBWID_license_key');
	} else {
		$base_url = admin_url( DS_PBWID_PLUGIN_PAGE );
